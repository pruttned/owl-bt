...
describe('Service: TreeModelConverter', function() {
  ...
  it('convertPropertiesToRuntime should convert property array to object', function() {
    let res = TreeModelConverter._convertPropertiesToRuntime([{
      name: 'prop1',
      value: 'value1'
    }, {
      name: 'prop2',
  ...
  it('convertSubItemToRuntime should clone subitem and convert its properties', function() {
    ...
    let res = TreeModelConverter._convertSubItemToRuntime(item);
  ...
  it('convertNodeToStore should clone node and all its subnodes+subitems; convert all property collections and remove meta inf + remove empty arrays/properties', function() {
    let node = {
      type: 'sequence',
      _meta: {
        id: 1,
    ...
    let res = TreeModelConverter.convertNodeToStore(node);
    expect(res).not.toBe(node);
    expect(res.decorators[0]).not.toBe(node.decorators[0]);
    expect(res.services[0]).not.toBe(node.services[0]);
    expect(res.childNodes[0]).not.toBe(node.childNodes[0]);
